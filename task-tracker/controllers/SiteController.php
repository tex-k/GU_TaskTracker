...
class SiteController extends Controller
{
    ...
    public function actionSql() {
        ...
        $update = Yii::$app->db->createCommand('UPDATE user SET login = "bbbb" WHERE id = :id')
            ->bindValue('id', 2)
            ->execute();

        $user = 'SELECT * FROM user WHERE id = :id';
        $users = Yii::$app->db->createCommand($user)->bindValue('id', 2)->queryOne();
        echo '<br>';
        print_r($users);
    }
    ...
    public function actionAr() {
        ...
        UserProfile::find()->where(['login' => 'ccc'])->one()->delete();
    }
}
